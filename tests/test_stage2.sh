...
EXPECTED_VERSION=$(grep '^version' moonstone.toml | head -1 | sed 's/.*"\(.*\)".*/\1/')
echo "$PKG_VERSION" | grep -q "$EXPECTED_VERSION" || { echo "FAIL: version mismatch (expected $EXPECTED_VERSION)"; exit 1; }
